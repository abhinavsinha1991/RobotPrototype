...
public class SimpleRobot implements Robot {
    ...
    public float walkDistance(int distance) {
        if (distance > MAX_DISTANCE_PER_CHARGE) {
            ...
            if ((currentCharge - (distance * PER_METRE_CHARGE_WITHOUT_WEIGHT)) >= 0.00f) {
                ...
                if (currentCharge <= 15.00f) batteryLowIndicator = true;
                System.out.println("Robot has moved " + distance
                        + " metres distance and battery status is "
                        + (batteryLowIndicator ? "low." : "adequate."));
            } else {
                System.out.println("Battery running at " + currentCharge
                        + "%.\nCharge reqd to fulfill request: "
                        + (distance * PER_METRE_CHARGE_WITHOUT_WEIGHT)
                        + "%. Please decrease distance.");
            }
        }
        return currentCharge;
    }
    ...
    public float walkWithWeight(int weight, int distance) {
        if (distance > MAX_DISTANCE_PER_CHARGE || weight > MAX_WEIGHT_CAP) {
            ...
            if (((currentCharge - ((distance * PER_METRE_CHARGE_WITHOUT_WEIGHT) + (2 * weight))) >= 0.00f)) {
                ...
                if (currentCharge <= 15.00f) batteryLowIndicator = true;
                System.out.println("Robot has moved " + distance
                        + " metres distance with weight " + weight
                        + " kgs and battery status is " + (batteryLowIndicator ? "low." : "adequate."));
            } else {
                System.out.println("Battery running at " + currentCharge
                        + "%.\nCharge reqd to fulfill request: "
                        + ((distance * PER_METRE_CHARGE_WITHOUT_WEIGHT) + (2 * weight))
                        + "%. Please decrease distance/weight.");
            }
        }
        return currentCharge;
    }
    ...
    public void scanItem() {
        ...
        if (random.nextBoolean()) {
            System.out.println("Item scanned successfully.Price is:" + String.format("%.02f", random.nextFloat()));
        } else {
